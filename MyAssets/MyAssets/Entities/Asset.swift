...
class AssetData: Identifiable, ObservableObject, Decodable {
    ...
    let title: String
    let amount: String
    //previous, current, nextmonth 결제 추가
...
//previous, current, nextmonth 결제 추가 Start
enum CreditCardAmounts: Identifiable, Decodable {
    ...
    //decodable 가능한 enum CodingKey 생성
    ...
    init(from decoder: Decoder) throws {
        ...
        throw fatalError("ERROR CreditCardAmounts JSON File Decoding")
    }
//previous, current, nextmonth 결제 추가 End
}
